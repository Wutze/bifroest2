...
# Broadcast DHCP
$FW -A INPUT -i $DEV_INTERN -s 0.0.0.0 -d 255.255.255.255 -p udp -m multiport --dport 67,68 -j ACCEPT
...
$FW -A INPUT -p tcp -i $DEV_INTERN -m multiport --dport 22,80,443 -j ACCEPT # ssh Dienst, Webserver intern erlauben
